...
void animateBallTranslation(float*,float*,float);
...
// Done with rendering now check for collision	 	
		//We should be releasing the collision here...
		detectCollision();
...
animateBall(float* transform, float* projection)
{
	...
	//	addDegrees = !addDegrees;
		...
		//If possible, turn this into a toggle.
		rotateBallAngle-=180;
		lockBallTranslate = true;
	}
	//True value*value > 0.95 * 0.95.  This might get around the if statement below
	if(lockBallTranslate && value > 0.95) {
		 rotateBallAngle-=90;
		 lockBallTranslate=false;
		 direction*=-1;
	}
	
	//JOSHUA!!!!!  YOU ALWAYS WANT POSITIVE DEGREES!!!
	//This is so you only need be positive for a sine-wave!
	if(value < 0)
		rotateBallAngle-=180;
