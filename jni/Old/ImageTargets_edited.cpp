...
Java_com_qualcomm_QCARSamples_ImageTargets_ImageTargetsRenderer_renderFrame(JNIEnv *, jobject)
{
    ...
    for(int tIdx = 0; tIdx < state.getNumActiveTrackables(); tIdx++)
    {
       ...
       QCAR::Matrix44F modelViewMatrix = QCAR::Tool::convertPose2GLMatrix(trackable->getPose());
		...
		float* renderTransform = &transformCourt.data[0];
...
/////////////////////////////////////////////////////////////////////////////////////////      
		...
		// Get transform from animate ball
       
		animateBall();
		
		
		LOG("transform:After");
		dumpMVM(&aniTransform.data[0]);
		
		
		float* temp=&ballTransform.data[0];
		// Now apply the transfom to ball transform and to render transform
		
		SampleUtils::multiplyMatrix(&aniTransform.data[0], //aniTransform is from animateBall()
									&ballTransform.data[0],
									temp);
		
		
		
		LOG("Ball transform:After");
		dumpMVM(&ballTransform.data[0]);
		
		
		renderTransform = &transformBall.data[0];
		SampleUtils::multiplyMatrix(&aniTransform.data[0],
									&transformBall.data[0],
									renderTransform);
		
		
		LOG("Ball Render transform:After");
		dumpMVM(renderTransform);
...
animateBall()
{
	aniTransform= SampleMath::Matrix44FIdentity();
...
// Detect collision and set flag "collision"	
